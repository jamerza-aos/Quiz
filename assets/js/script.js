...
figure.append(img)
let imgtext= document.createTextNode(output.question)
let figcaption= document.createElement ('figcaption')
figcaption.append(imgtext)

let div= document.getElementById('cotent').append(figure)

figure.append(figcaption)
let div1 =document.createElement('div')

for ( let j =0; j< output.choice.length; j++)
{
    let button = document.createElement('button')
    button.addEventListener('click' ,(event) => {
        console.log("32")

        if( output.choice[j]== data[i].answer)
        {
    ...
    let answertext= document.createTextNode(output.choice[j])
...
body.style.background= "rgba(30, 139, 255, 0.317)"
//body.append(div)
}
